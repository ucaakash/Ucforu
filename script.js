...
let liveSettings = { 
    ...
    whatsappText: "⚔️ {name} challenged you!\nMy screen time: {time}\nCan you beat me? 😎\n\n{link}"
};
...
document.getElementById('analyzeBtn').addEventListener('click', async () => {
    ...
    const loadingMessages = [
        ...
        "Drafting the perfect insult... ✍️", 
        "Calculating your wasted hours... ⏳", 
...
function setupDownloadAndShare(imgUrl) {
    ...
    document.getElementById('shareBtn').onclick = async () => { 
        try { 
            const blob = await (await fetch(imgUrl)).blob(); const file = new File([blob], "DigitalRhythm.png", { type: "image/png" }); 
            if (navigator.canShare && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'My Digital Rhythm', url: 'https://ucforu.online' }); } 
...
function showToast(message, type = "info") {
    ...
    container.appendChild(toast); setTimeout(() => toast.classList.add("show"), 100); setTimeout(() => { toast.classList.remove("show"); setTimeout(() => toast.remove(), 400); }, 3000);
}
    ...
    if (challengeBtn) {
        challengeBtn.addEventListener("click", async () => {
            try {
                ...
                let shareText = liveSettings.whatsappText || "⚔️ {name} challenged you!\nMy screen time: {time}\nCan you beat me? 😎\n\n{link}";
                shareText = shareText.replace("{name}", currentNickname).replace("{time}", `${Math.floor(currentUserTime/60)}h ${currentUserTime%60}m`).replace("{link}", challengeLink);
