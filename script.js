...
document.getElementById('analyzeBtn').addEventListener('click', async () => {
    ...
    let msgIndex = 0;
    if(loadingTextEl) loadingTextEl.innerText = loadingMessages[msgIndex];
...
function handleBattleResult() {
    ...
    const card = document.createElement("div"); card.className = "flower-battle";
    card.innerHTML = `<div class="battle-title">Screen Time Battle</div><div class="battle-row"><div class="player-box"><div class="player-name">${window.challengeData.creator}</div><div class="player-time">${creatorH}h ${creatorM}m</div></div><div class="vs">VS</div><div class="player-box"><div class="player-name">${currentNickname}</div><div class="player-time">${userH}h ${userM}m</div></div></div><div class="winner-text">👑 Winner: ${winner}</div>`;
    document.getElementById("result").prepend(card);

    if (database && window.challengeID) { database.ref("challenges/" + window.challengeID).update({ opponent: currentNickname, opponentTime: currentUserTime, winner: winner }); }
}

function saveLeaderboardData(timeString) {
    if (database) {
        const dailyKey = getLocalDailyKey(); const safeName = currentNickname.replace(/[^a-zA-Z0-9]/g, "User"); const userKey = safeName + "_" + Date.now();
        database.ref(`leaderboard/${dailyKey}/${userKey}`).set({ nickname: currentNickname, totalMinutes: currentUserTime, formattedTime: timeString, deviceId: myDeviceId, timestamp: firebase.database.ServerValue.TIMESTAMP }).catch(err => console.error("Firebase Save Error:", err));
    }
}

function loadLeaderboard() {
    if (!database) return;
    const dailyKey = getLocalDailyKey(); const tbody = document.getElementById('leaderboardBody');
    database.ref('leaderboard/' + dailyKey).on('value', (snap) => {
        tbody.innerHTML = "";
        if (snap.exists()) {
            let uniqueEntries = {}; 
            snap.forEach((child) => { 
                let data = child.val(); let groupingKey = (data.deviceId || "nodevice") + "_" + data.nickname.toLowerCase().trim();
                if (!uniqueEntries[groupingKey] || uniqueEntries[groupingKey].totalMinutes < data.totalMinutes) uniqueEntries[groupingKey] = data;
            });
            let results = Object.values(uniqueEntries); results.sort((a, b) => b.totalMinutes - a.totalMinutes);
            let top10 = results.slice(0, 10);
            if (top10.length > 0) { top10.forEach((data, index) => { const medal = index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : (index + 1); tbody.innerHTML += `<tr><td>${medal}</td><td>${data.nickname}</td><td style="text-align: right;">${data.formattedTime}</td></tr>`; });
            } else tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;">No data yet</td></tr>';
        } else tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;">No data yet</td></tr>';
    });
}

function setupDownloadAndShare(imgUrl) {
    document.getElementById('downloadBtn').onclick = () => { const link = document.createElement('a'); link.download = `UCforU_${currentNickname}.png`; link.href = imgUrl; link.click(); };
    document.getElementById('shareBtn').onclick = async () => { try { const blob = await (await fetch(imgUrl)).blob(); const file = new File([blob], "DigitalRhythm.png", { type: "image/png" }); if (navigator.canShare && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'My Digital Rhythm', url: 'https://ucforu.online' }); } else alert("Share feature not supported on this device."); } catch (e) { console.error(e); } };
}
...
window.addEventListener("DOMContentLoaded", () => {
    if(database) {
        database.ref('app_settings').on('value', (snap) => {
            if(snap.exists()) {
                ...
                else { if (maintenanceMsg) maintenanceMsg.style.display = 'none'; if (inputForm && (!resultBlock || resultBlock.style.display !== 'block')) inputForm.style.display = 'block'; }

                const marqueeContainer = document.getElementById('live-marquee'); const marqueeText = document.getElementById('marquee-text');
                if(liveSettings.announcementText && liveSettings.announcementText.trim() !== "") { if(marqueeContainer) marqueeContainer.style.display = 'block'; if(marqueeText) marqueeText.innerText = liveSettings.announcementText; } 
    ...
    if (window.challengeID && database) {
        const banner = document.createElement("div"); banner.style.cssText = "background:rgba(0,0,0,0.3); padding:15px; border-radius:12px; margin-bottom:20px; text-align:center;"; banner.innerHTML = `<h3>⚔️ Loading challenge...</h3>`; document.querySelector("main").prepend(banner);
        database.ref("challenges/" + window.challengeID).once("value").then((snap) => {
            if (snap.exists()) { window.challengeData = snap.val(); banner.innerHTML = `<h3>⚔️ Challenge From ${window.challengeData.creator}</h3><p>Screen Time: ${Math.floor(window.challengeData.creatorTime/60)}h ${window.challengeData.creatorTime%60}m</p><p>Upload your screenshot to beat this score 🏆</p>`; if (guideModal) guideModal.querySelector("h2").innerHTML = "Accept Challenge! ⚔️"; } 
            else banner.innerHTML = `<h3>Challenge not found</h3>`;
        });
    }
    ...
    const tabAndroid = document.getElementById('tabAndroid'), tabIphone = document.getElementById('tabIphone'); const contentAndroid = document.getElementById('contentAndroid'), contentIphone = document.getElementById('contentIphone');
